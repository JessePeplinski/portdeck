...
That local candidate is an unsandboxed, ad-hoc-signed packaging proof. Provider CLIs remain external in both local and production candidates. The App Store build compiles launcher controls out until the bundled-helper, Node JIT entitlement, and process-control design are separately proven under App Sandbox. Source, local-candidate, future Developer ID direct-download, and App Store paths all keep the same status JSON boundary so CLI dogfooding, app rendering, and future automation stay aligned.
