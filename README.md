...
> [!IMPORTANT]
> PortDeck is in pre-release development. `v0.1.0-beta.8` is the next signed and notarized GitHub prerelease target. Source-development, sandbox-probe, local release-candidate, and production direct-download artifacts remain separate workflows.
...
- [`PortDeck-0.1.0-beta.8-macos-arm64.dmg`](../../releases/download/v0.1.0-beta.8/PortDeck-0.1.0-beta.8-macos-arm64.dmg)
- [`PortDeck-0.1.0-beta.8-macos-arm64.dmg.sha256`](../../releases/download/v0.1.0-beta.8/PortDeck-0.1.0-beta.8-macos-arm64.dmg.sha256)

Download both files into the same directory, verify the disk image, then open it:

```bash
shasum -a 256 -c PortDeck-0.1.0-beta.8-macos-arm64.dmg.sha256
open PortDeck-0.1.0-beta.8-macos-arm64.dmg
```
